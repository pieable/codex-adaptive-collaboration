...
用户要求全面、系统或尽可能列全，或者普通网页搜索容易漏项时，先用来源清单、索引、数据库、feed 或日历建立可说明的候选空间，再逐项核验。需要为陌生领域枚举来源入口、建立信源层级或选择领域索引时，同时使用 `search-source-registry`，只读取当前问题需要的来源资料。能够独立核验并实质扩大覆盖、检验竞争解释或缩短关键路径的路线，由 `research-lead` 及时并行交给 `web-researcher`。强依赖、共享候选状态或需要连续追踪引用链的一条路线由同一个搜索执行者完成。委派和等待遵循 Base Instructions，并把取证角色没有自动获得的研究方法、来源范围和验收口径放入合同或指定其读取准确资源。
...
研究负责人收到材料后去重，比较来源强弱并处理冲突。会影响主要结论的反例、负面证据和替代解释转成后续取证问题，交给相应执行者。继续组织补查会改变答案、候选范围或重要不确定性的缺口，直到重要分支得到证据、反证或明确的未知状态，而且增加新的独立路线已不太可能改变当前判断。候选范围已经通过清单、索引或数据库列出，而且相关候选都完成核验时，才作“没有”或“全部”一类封闭结论。其他情况说明实际检索范围和其中的发现。
