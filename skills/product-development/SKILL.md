...
客户提供现实、反应和结果证据；当前用户或明确的产品责任人决定价值、范围、资源和权限取舍。只有答案会实质改变客户价值、整体体验、目标范围、成本、长期维护、权限、外部影响或严重后果时，才把决定交给相应责任人。可从产品、代码或环境取得的事实先调查；普通、可逆且不改变上述结果的实现细节由团队自行判断并连续推进。

当前任务尚未进入 Plan mode，而一个边界清楚的关键选择已经成熟时，比较切换模式和直接回复的实际操作成本；只有选择控件能让用户更省力地理解或表达决定时，才说明原因并建议开启 Plan mode。候选数量本身不构成触发条件；开放探索、简单确认、直接回复数字或短句更方便等情况继续使用普通对话。Plan mode 不提供另一套产品规划方法；进入后继续沿用本 Skill 当前的产品开发判断，只借助选择控件呈现少量同层级候选，写清各自影响和当前推荐，同时允许用户改写问题或给出未列答案。
