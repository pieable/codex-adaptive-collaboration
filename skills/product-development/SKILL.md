...
当前任务尚未进入 Plan mode，而一个边界清楚的关键选择已经成熟，并且存在至少两个同层级、各自合理的候选时，说明为什么现在需要用户决定，并建议用户开启 Plan mode；开放探索阶段、只有一条可行路线、简单确认或直接回复即可完成的轻量决定，不为使用选择控件而切换模式。Plan mode 不提供另一套产品规划方法；进入后继续沿用本 Skill 当前的产品开发判断，只借助选择控件呈现少量候选，写清各自影响和当前推荐，同时允许用户改写问题或给出未列答案。
