...
把本仓库安装到当前用户的 Codex 配置目录，使 Root Base、共同执行层、九个角色和九个体系 Skill 在新任务中可用。
...
1. 读取 `examples/config.toml`、`prompt-lab/`、`global/AGENTS.md`、`agents/` 和 `skills/`，确认仓库结构完整。
...
4. 把 `global/AGENTS.md` 的共同执行层合并到 `CODEX_HOME/AGENTS.md`。保留已有且与本体系无关的内容，已经存在的相同规则不重复写入。
5. 把示例和角色文件中出现的 `{{CODEX_HOME}}` 替换为真实绝对路径。只替换这个占位符，不改写其他指令内容。
6. 合并 `config.toml`：
   - 保留无关顶层配置；
...
7. 如果示例模型不可用，列出 Root、Luna 执行、Terra 阶段与评审、Spark Worker 四个能力层级，请用户确认映射；不要自行选择实质不同的模型。
8. 预检目标宿主和父配置：`browser-operator` 需要浏览器能力，`visual-usability-tester` 需要 Computer Use。若它们未启用或目标版本不支持，说明这两个角色不可用；启用这类真实交互能力前先取得用户确认，不要因为安装而自动扩大权限。
9. 解析合并后的 TOML，检查所有目标文件存在，并确认配置引用的路径都能解析。
10. 新建一个 Root 测试任务和至少一个 subagent 测试任务，验证真实运行时已加载新 Base、共同执行层和角色指令。可用时分别验证浏览器角色和视觉测试角色的实际工具；无法进行真实加载测试时，明确写“未验证”，不要用文件存在代替。
