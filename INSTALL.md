...
5. 合并 `config.toml`：
   ...
   - 创建或替换本仓库点名的六个 `[agents.<role>]` 注册段；
   - 保留其他 Agent、MCP、插件和功能配置。
6. 如果示例中的模型名称不可用，列出 Root、轻量执行、阶段负责人和高速 Worker 四个能力层级，请用户确认映射；不要自行选择实质不同的模型。
7. 运行共享规则检查：

   ```powershell
   pwsh -NoProfile -File <CODEX_HOME>/prompt-lab/sync-subagent-instruction-blocks.ps1 -Check
   ```

8. 解析合并后的 TOML，检查所有目标文件存在，并确认配置引用的路径都能解析。
9. 新建一个 Root 测试任务和至少一个 subagent 测试任务，验证真实运行时已加载新 Base 和角色指令。无法进行真实加载测试时，明确写“未验证”，不要用文件存在代替。
...
| 阶段负责人 | `gpt-5.6-terra` | Research Lead、Code Executor |
